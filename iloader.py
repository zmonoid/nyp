...
class ImageList(data.Dataset):
    def __init__(self,
        ...
        with open(list_path, 'r') as f:
            ...
            for line in lines:
                ...
                angle = torch.torch.FloatTensor([angle])
                intention_ = int(line.split(' ')[-1].strip())
                intention = torch.eye(num_intention)
                intention = intention[intention_, :]
                item = (path, angle, intention)
                imgs.append(item)
    ...
    def __getitem__(self, index):
        path, angle, intention = self.imgs[index]
